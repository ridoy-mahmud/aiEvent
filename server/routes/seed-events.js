...
// @desc    Add all 38 events to database (only adds missing ones)
// @access  Public (or add protect, admin if you want to secure it)
  ...
  try {
    ...
    if (!admin) {
      return res.status(404).json({
        success: false,
        error: 'Admin user not found. Run /api/init first.',
      });
    }
    ...
    // Find missing events
    const missingEvents = allEvents.filter(e => !existingTitles.has(e.title));
    
    if (missingEvents.length === 0) {
      return res.json({
        success: true,
        message: 'All 38 events already exist in database',
        data: {
    ...
    res.json({
      success: true,
      message: `Added ${missingEvents.length} events to database`,
      data: {
